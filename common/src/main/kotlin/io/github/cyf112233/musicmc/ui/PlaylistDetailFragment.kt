...
class PlaylistDetailFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: DataSet?): View {
        ...
        songCountText = TextView(context).apply {
            text = if (songs.isEmpty()) UiText.t("加载中…", "Loading…") else UiText.t("${songs.size} 首歌曲", "${songs.size} songs")
            setTextSize(12f)
        ...
        if (playlist.songs.isEmpty()) {
            ...
            NetMusic.source.playlistDetail(playlist.id) { detail, err ->
                Async.onUi {
                    ...
                    if (err != null) {
                        loading.text = UiText.t("加载失败: $err", "Failed to load: $err")
                        loading.visibility = View.VISIBLE
                    } else if (detail.songs.isEmpty()) {
                        loading.text = UiText.t("歌单为空", "Playlist is empty")
                        loading.visibility = View.VISIBLE
                    } else {
                        songs = detail.songs
                        songCountText?.text = UiText.t("${detail.songs.size} 首歌曲", "${detail.songs.size} songs")
